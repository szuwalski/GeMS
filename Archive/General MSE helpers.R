...
if(plots==1)
{
plot(IndexData,ylim=c(0,max(IndexData)),las=1)
lines(predBio)
lines(CatchData,lty=2,col=2)
}
return(predBio)
}
...
cnt<-grep("srv_q",data2)
srvQ<-as.numeric(data2[cnt+1])
...
SurvSel<-srvQ/( 1 + exp( -1*log(19)*(AgeBin-sel50surv)/(sel95surv-sel50surv)))
...
lines(SurvSel~AgeBin,lty=1,col=2)
