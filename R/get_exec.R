...
get_exec <- function(exe_name=SimAssExec) {
  exec.path <- system.file("bin", package = "GeneralMSE")
  full.path <- file.path(exec.path,exe_name)
  if(!file.exists(full.path)) stop("Executable file not found. Please download and install package from https://github.com/szuwalski/General-MSE")
  return(full.path)
}
